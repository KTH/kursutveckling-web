  ...
  return (
    <span>
      <span className='navigation'>
        <h3>{year}</h3>
        <a href={`${KUTV_ADMIN_URL}${courseCode}?l=${koppsData.lang}&status=n&serv=kutv`}>{translate.header_main_publish_new}</a>
      </span>
    ...
    <div className='tables-list col'>
      {
        yearsDescending.map((year, index) =>
          <CourseDevAllRoundsForEachYear key={index} arrOfRoundObj={allYearsObj[year]} koppsData={koppsData} courseCode={courseCode} year={year} translate={translate} />
        )
      }
    </div>
  )
